...
export async function saveListing(userId: string, input: ListingDraft): Promise<{ ok: true; id: number; status: string } | { ok: false; errors: string[] }> {
  ...
  if (input.intent === "publish") {
    if (!whatsapp || whatsapp.length < 8 || whatsapp.length > 15) errors.push("Add a WhatsApp number with the country code.");
  ...
  for (let i = 0; i < photos.length; i += 1) {
    ...
    if (!photo) continue;
    await sql`
      insert into vehicle_images (listing_id, url, alt, sort_order, is_primary)
      values (${listingId}, ${photo.url}, ${cleanText(photo.alt, 140) || title}, ${i}, ${i === 0})
    `;
  }
